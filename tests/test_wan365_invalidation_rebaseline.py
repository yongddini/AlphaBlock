...
_UNPINNED_BY_DESIGN: dict[str, str] = {
    ...
    # 존의 두께 층 재측정(WAN-376) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야
    # §0 지도와 §1a 지름길이 **오늘 엔진**의 것이다). `assert_adopted_base`가 취소 시점
    # 기본값이 인과가 아니면 실행 자체를 거부한다.
    "wan376_zone_thickness.py": "인과 엔진 재측정 · 기본값 감시가 내장",
    ...
    # 진입 순간 MACD 색 관측(WAN-372) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야
    # 색을 붙이는 거래가 **오늘 엔진이 실제로 한 거래**다). 관측 팔 ≡ 인자 없는 채택 북을
    # 실데이터 검산 (a)로 못 박는다(색은 순수 관측이라 그 등식이 정확히 0이어야 한다).
    "wan372_macd_color.py": "관측 팔 ≡ 인자 없는 채택 북 (검산)",
    # 확인 진입 트리거 관측(WAN-383 §1) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야
    # 트리거를 재는 거래가 **오늘 엔진이 실제로 한 거래**이고, 대기 창의 끝인 「존 무효화」도
    # 오늘 규칙이다). 관측 팔 ≡ 인자 없는 채택 북을 실데이터 검산 (a)로 못 박는다.
    "wan383_confirmation_entry.py": "관측 팔 ≡ 인자 없는 채택 북 (검산)",
    # 하루 손실 서킷브레이커(WAN-410) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야
    # 서킷브레이커가 재는 그 손실이 **오늘 엔진이 실제로 낸 손실**이다). 「멈춤 없음」 팔 ≡
    # 인자 없는 채택 북을 실데이터 검산 (a)로 못 박는다(`0.00e+00`).
    "wan410_daily_loss_circuit_breaker.py": "멈춤 없음 팔 ≡ 인자 없는 채택 북 (검산)",
    # 방향 팔 셋(WAN-412) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야 세 팔이
    # **오늘 엔진** 위의 방향 대조다. 옛 숏 표(WAN-282/288/301)가 얼어붙은 이유가 정확히 그
    # 취소 시점이라, 핀을 걸면 이 이슈가 존재할 이유 자체가 사라진다). `long_only` 팔 ≡
    # 인자 없는 채택 북을 검산 (a)로 못 박는다.
    "wan412_short_direction_book.py": "long_only 팔 ≡ 인자 없는 채택 북 (검산 a)",
    # 확인 진입 손익 격자(WAN-386 §3) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야
    # 팔이 갈아끼우는 그 셋업이 **오늘 엔진이 실제로 하는 매매**다). 기준 팔 후보 ≡ 엔진
    # base+재진입을 검산 (a-1)로, 그 지갑 ≡ 채택 북 지갑을 (a-2)로 못 박는다.
    "wan386_confirmation_pnl.py": "기준 팔 ≡ 인자 없는 채택 북 (검산 a-1·a-2)",
    # 존 병합 × 재탭 2×2(WAN-388 §2) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야
    ...
    # 재탭 × 재진입 × 익절 배수 세 축(WAN-394 §1) — WAN-389/381의 후속이라 **같은 이유로**
    # 핀이 없다(WAN-305 · 인과 취소를 물려받아야 16점이 오늘 엔진 위의 격자다). 그리고 이
    # 모듈은 자기 1.5R 네 칸이 **WAN-389 공개 CSV와**, `every_tap` × 재진입 ON의 배수 4점이
    # **WAN-381 공개 CSV와** 비트 일치함을 검산 (a)·(b)로 못 박으므로, 핀을 걸면 그 두 등식이
    # 오히려 깨진다(둘 다 핀 없이 낸 표다).
    "wan394_retap_reentry_tp.py": "1.5R 네 칸 ≡ WAN-389 CSV · 배수 4점 ≡ WAN-381 CSV (검산 a·b)",
    # 익절 배수 꺾임(WAN-395) — WAN-381의 후속이라 **같은 이유로** 핀이 없다(WAN-305 · 인과
    # 취소를 물려받아야 여섯 점이 오늘 엔진 위의 공선이다). 그리고 이 모듈은 자기 채택 점이
    # **인자 없는 채택 북과** 비트 일치함을 검산 (a-2)로, 겹치는 배수 4점이 **WAN-381 공개
    # CSV와** 비트 일치함을 (d)로 못 박으므로, 핀을 걸면 그 두 등식이 오히려 깨진다(WAN-381도
    # 핀 없이 낸 표다).
    "wan395_exit_multiple_inflection.py": (
        "채택 점 ≡ 인자 없는 채택 북 · 배수 4점 ≡ WAN-381 CSV (검산 a-2·d)"
    ),
    # 슬리피지 실측(WAN-397) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야 손절 봉이
    # **오늘 엔진이 실제로 낸 손절**이고 요율 민감도도 오늘 좌표의 것이다). 그리고 이 모듈은
    # 자기 기준 팔(5bp)이 **WAN-395 공개 CSV의 채택 점과** 비트 일치함을 검산 (a)로 못 박으므로,
    # 핀을 걸면 그 등식이 오히려 깨진다(그쪽도 핀 없이 낸 표다).
    "wan397_stop_slippage.py": "기준 팔 ≡ WAN-395 CSV의 채택 점 (검산 a)",
    # 무효화 봉 「같은 존 연속 체결」 인구조사(WAN-409) — 🚨 핀 없음이 **이 이슈의 전제**다.
    # 이 표가 세는 무더기의 94.5%가 「무효화 봉 안 체결」인데, 그 거래는 **인과 취소에서만
    # 존재한다**(옛 소급 취소에서는 정의상 0건). 핀을 걸면 재려는 현상 자체가 사라진다.
    # 그리고 이 모듈은 자기 기준 팔이 **인자 없는 채택 북과** 비트 일치함을 검산 (a)로,
    # **공개 `book_trades.csv` 집계와** 일치함을 (c)로 못 박는다 — 핀을 걸면 둘 다 깨진다.
    # ⚠️ 옛 취소 시점은 이 모듈 안에서 **반사실 팔로 명시**된다(`cancel_bar_open`).
    "wan409_invalidation_cascade.py": "기준 팔 ≡ 인자 없는 채택 북 · ≡ 공개 CSV (검산 a·c)",
    # 손실 집중·동시성 관측(WAN-408 §0) — 🚨 핀 없음이 **이 표의 전제**다. 이 모듈은 「인자
    # 없는 채택 북이 실제로 한 거래」를 다시 집계할 뿐이라, 옛 취소 시점을 박으면 세는 대상이
    # 오늘 매매가 아니게 된다(그리고 검산 (c) 「≡ 공개 `book_trades.csv` 집계」가 깨진다 —
    # 그 CSV가 인과 취소 위의 산출물이다). 흔드는 축이 **하나도 없는** 관측 모듈이다.
    "wan408_loss_clustering.py": "≡ 공개 book_trades.csv 집계 (검산 c) · 관측 전용",
    # 조건부 손익비 Phase 1 관측(WAN-375) — 핀 없음이 설계다(WAN-305 · 인과 취소를 물려받아야
    # 재는 경로가 **오늘 엔진이 실제로 거는 셋업**의 것이다). 익절만 끈 판의 관측으로 되살린
    # 익절 켠 판이 **인자 없는 채택 엔진 후보와 같음**을 칸마다 검산 (b)로 못 박는다 — 핀을 걸면
    # 그 등식이 옛 엔진과의 대조가 된다.
    "wan375_conditional_rr.py": "익절 켠 판 복원 ≡ 채택 엔진 후보 (검산 b) · 관측 전용",
    # WAN-402 §0 `concurrent_breaks` — 엔진을 부르지 않는다(WAN-375 셋업 표를 읽고 탐지 층만
    # 새로 만든다). 인과 취소는 그 모집단과 존 대장(`break_time` = 인과 엔진의 무효화 봉)에
    # 이미 실려 있어 핀을 걸 자리가 없다 — 옛 취소 시점을 박으면 「알려진 시각」 자체가 뜻을 잃는다.
    "wan402_concurrent_breaks.py": "WAN-375 셋업 표 + 탐지 층 대장 · 관측 전용 (검산 d)",
    # WAN-424 §1(스토캐스틱 × 오더블록 팔 복원) — 핀 없음이 설계다(WAN-305). WAN-423 §6·§7이
    # 인과 취소(오늘 엔진) 위에서 낸 표를 재현하는 모듈이라, 옛 취소 시점을 박으면 재현 대상
    # 자체가 달라진다. 재현 검산 90/90과 파일럿 후보 단위 대조가 그 등식을 못 박는다.
    "wan424_stoch_ob_arm.py": "WAN-423 §7 표 재현(90/90) · 인과 엔진 위 팔",
    # WAN-424 §2 매칭 대조군 — §1 팔 위에서 대조군을 거는 모듈이라 같은 이유로 핀 없음이 설계다.
    "wan424_matched_null.py": "§1 팔(인과 엔진) 위 대조군 · 실제 팔 ≡ §1 재현값 검산",
    # WAN-424 §3 격리 — 같은 팔에서 손절폭 하한만 흔들어 %K의 몫을 떼는 모듈이라 같은 이유다.
    "wan424_stoch_isolation.py": "§1 팔(인과 엔진) 위 하한 격리 · 하한 4% 행 ≡ §1 재현값",
    # WAN-428 부록 — `wan424_stoch_ob_arm`의 팔 후보를 **그대로 빌려** 순위 라벨과 `Zone Count`
    # 캡만 더한다. 취소 시점은 그쪽(인과 엔진)이 정하고 이 모듈은 건드리지 않는다. 검산 (a′)가
