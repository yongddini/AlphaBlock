...
def test_baseline_lens_is_the_adopted_default_engine() -> None:
    """`baseline` 렌즈 = 채택 기본값 그 자체여야 WAN-108 행이 재현된다.

    이 리포트의 `baseline` 열은 WAN-108 표와 **같은 숫자**여야 대조가 성립한다.
    보수화 노브를 뺀 나머지가 하나라도 다르면 그 대조가 깨진다.
    """
    assert build_params(fill=fill_preset("baseline"), seed=0) == PARAMS
...
def test_build_cell_lens_params_differ_from_default() -> None:
    """렌즈 파라미터는 기본값과 달라야 한다(같으면 축이 아무 일도 안 한 것)."""
    lens = build_params(fill=fill_preset("pen_5bp"), seed=0)
    assert lens != PARAMS
    assert lens.fill_penetration_bps == 5.0
    # 보수화 노브 **말고는** 채택 기본값과 같아야 한다.
...
def test_default_params_untouched_by_this_report() -> None:
    ...
    assert ConfluenceParams() == PARAMS
