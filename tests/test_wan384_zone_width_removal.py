...
_NO_PIN_NEEDED: frozenset[str] = frozenset(
    {
        ...
        # WAN-409도 같다 — **필터 꺼진 채택 북**을 그대로 돌면서 「직전 거래와 같은 존인가」를
        # 세는 인구조사이고, 반사실 팔도 그 위에서 축을 하나씩만 흔든다(핀 없음이 설계,
        ...
        # WAN-375 Phase 1도 같다 — **필터 꺼진 채택 엔진**의 셋업에서 익절만 끈 경로를 관측하는
        # 표라 핀 없음이 설계다(WAN-305). 검산 (b)가 익절 켠 판 복원 ≡ 채택 엔진 후보를 못 박는다.
        "wan375_conditional_rr",
        # WAN-402 §0(`concurrent_breaks`)도 같다 — 엔진을 안 돌린다: WAN-375의 **필터 꺼진 채택
        # 엔진** 셋업 표를 모집단으로 읽고 탐지 층(존 대장)만 새로 만든다. 핀을 걸 자리가 없고,
        # 검산 (d)가 그 캐시 한 칸을 오늘 코드로 다시 만들어 같음을 못 박는다.
        "wan402_concurrent_breaks",
        # WAN-417 §G-0(OI 사건 정렬)도 같다 — 엔진을 안 돌린다: WAN-402와 **같은 모집단**(WAN-375
        # 필터 꺼진 채택 엔진 셋업 표)을 읽고 거래소 metrics 아카이브(OI)만 붙인다. 핀을 걸 자리가
        # 없고, 검산 (a)·(b)가 모집단 ≡ WAN-375 표를 값으로 못 박는다.
        "wan417_oi_event_study",
        # WAN-419(양봉 확인 진입 §0·§1)도 같다 — 엔진을 안 돌린다: WAN-402/417과 **같은 모집단**
        # (WAN-375 필터 꺼진 채택 엔진 셋업 표)을 읽고 상위TF 봉 색만 붙인다. 핀을 걸 자리가
        # 없고, 검산 (a)·(b)가 모집단 건수 ≡ WAN-375 표 · 셋업 원열 불변을 값으로 못 박는다.
        "wan419_bullish_confirmation",
        # WAN-428 §1(존 순위 인구조사)은 wan408/wan409와 **정확히 같은 부류**다 — **필터 꺼진
        # 채택 북**을 그대로 돌면서(후보 생성·배치를 `wan408`에서 빌려 쓴다) 거래마다 「그 시점
        # 몇 위 존이었나」만 붙이는 관측이라 핀 없음이 설계다(WAN-305). `zone_limit`을 시그널
        # 경로에 걸지 않았고 축도 하나 안 흔든다. 검산 (a)가 공개 `book_trades.csv` 집계와의
        # 일치를, (c)가 존 인덱스 매핑을 값으로 못 박는다.
        "wan428_zone_rank_census",
        # WAN-428 부록(스토캐스틱 팔 순위·캡)도 같은 부류다 — 팔 후보를 `wan424_stoch_ob_arm`에서
        # **그대로 빌려 오고**(그쪽이 존폭 축을 이미 정한다: 필터 대신 **손절폭 하한**을 쓴다) 이
        # 모듈이 더하는 것은 순위 라벨과 `Zone Count` 캡뿐이다. 핀 없음이 설계(WAN-305)이고,
        # 검산 (a′)가 WAN-424 §1 기준값 재현으로 팔 동일성을 값으로 못 박는다.
        "wan428_zone_rank_stoch_arm",
        # WAN-428 §2(채택 북 `Zone Count` 캡)도 §1과 같은 부류다 — **필터 꺼진 채택 북**을
        # 그대로 돌면서(후보 생성·배치를 `wan408`에서 빌려 쓴다) 후보를 존 순위로만 거른다.
        # 존폭 문턱을 박으면 무제한 팔이 「인자 없는 채택 북」이 아니게 되어 검산 (a)가 거짓이
        # 된다(핀 없음이 설계, WAN-305). (a)가 거래 수·거래당 net R ≡ 공개 채택 북을 값으로
        # 못 박고, (d)가 캡 감소율 ↔ 순위 몫의 차 = **재배치의 크기**를 관측으로 남긴다.
        "wan428_zone_cap_book",
        # WAN-421(존이 뚫리기 전 맥락 네 축)도 같다 — 엔진을 안 돌린다: WAN-402/417/419와 **같은
        # 모집단**(WAN-375 필터 꺼진 채택 엔진 셋업 표)을 읽고 상위TF 봉 라벨만 붙인다. 존 경계는
        # 기본 `OrderBlockParams()` 탐지로 읽는데 존폭 필터는 탐지 파라미터가 아니다. 검산
        # (a)·(b)·(f)가 모집단 건수 ≡ WAN-375 표 · 셋업 원열 불변 · 존 바닥 ≡ 손절가를 못 박는다.
        "wan421_pre_break_context",
        # WAN-410도 같다 — **필터 꺼진 채택 북**을 그대로 돌면서 「그날 실현손실이 문턱을
        # 넘으면 그 뒤 진입을 멈춘다」를 얹는 격자라 핀 없음이 설계다(WAN-305 · 필터를 켠
        # 채 낸 표가 아니다). 검산 (a)가 「멈춤 없음」 팔 ≡ 인자 없는 채택 북을 `0.00e+00`로
        # 못 박고, (d)가 서킷브레이커는 **배치 축**이라 후보를 안 바꿈을 못 박는다.
        "wan410_daily_loss_circuit_breaker",
        # 방향 팔 셋(WAN-412)도 같다 — 필터를 켠 채 낸 표가 아니라 **필터 꺼진 채택 북** 위에서
        # 방향(롱/숏)만 갈아끼운 표다(핀 없음이 설계, WAN-305 · 존폭 문턱을 박으면 세 팔이 오늘
        # 엔진이 아닌 판을 돈다). 검산 (a)가 「숏 게이트가 롱 후보를 안 건드린다」를 base 후보
        # 파리티 0/48로 못 박고, (b)/(c)가 방향 필터와 숏 기하를 동작으로 고정한다.
        "wan412_short_direction_book",
        # WAN-424 §1도 같다 — **필터 꺼진 채택 엔진**의 base 후보 위에서 청산만 시간 청산으로
        # 갈아끼우고 손절폭 하한·%K로 거르는 팔이다(핀 없음이 설계, WAN-305). 존폭 문턱을 박으면
        # 재현 대상인 WAN-423 §7 표(필터 꺼진 판)가 아닌 판을 돈다 — 재현 검산 90/90이 못 박는다.
        "wan424_stoch_ob_arm",
