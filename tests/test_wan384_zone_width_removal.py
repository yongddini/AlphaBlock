...
_NO_PIN_NEEDED: frozenset[str] = frozenset(
    {
        ...
        "wan424_stoch_ob_arm",
        # WAN-430(그 팔의 15m + TF별 분해)도 같다 — 후보·팔·필터·배치를 `wan424_stoch_ob_arm`
        # 에서 **그대로 빌려 오고**(그쪽이 존폭 축을 이미 정한다: 필터 대신 **손절폭 하한**) 이
        # 모듈이 더하는 것은 **TF 하나와 스코프 축**뿐이다. 존폭 문턱을 박으면 재현 대상인
        # WAN-424 §1 판(필터 꺼진 판)이 아닌 판을 돌아 **검산 (a′)가 통째로 깨진다**.
        "wan430_stoch_arm_15m",
