...
산식**을 쓰게 한다. 두 경로가 서로 다른 상수를 들고 있으면 라이브·백테스트 대조
...
- **지정가(메이커) 체결(B안)**: 메이커 수수료가 붙고, **슬리피지는 원칙적으로 없다**
  ...
  `fill_rate`로 추적한다). ⚠️ **청산은 「항상 테이커」가 아니다 — 사유별로 갈린다**
  (WAN-370): 익절(부분 익절 포함)은 지정가 reduce-only 집행이라 **메이커·슬리피지 0**,
  손절·만료·데이터 종료는 시장가 성격이라 테이커다. 분기의 단일 소스는
  `backtest.models.BacktestConfig.exit_liquidity(reason)`이고, 이 모듈은 그 판정 결과
  (`Liquidity`)를 받아 요율·슬리피지를 매길 뿐이다.
