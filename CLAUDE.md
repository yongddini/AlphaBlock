...
📌 **공식 체결 기준선 · 채택 TF(WAN-97 확정)**: [`docs/decisions/wan97.md`](docs/decisions/wan97.md).
**모든 채택 판단과 리포트의 주 수치는 `pen_5bp`**(지정가 5bp 관통 요구, 탈락 0%)로 낸다 —
`baseline`("닿으면 체결")은 큐 우선순위를 무시한 낙관이라 **단독 인용 금지**이고,
`pen_5bp_drop_50`은 탈락률 50%가 임의값에 시드 잡음이 커 기준선이 못 된다. 세 값을
`baseline`(상한) · **`pen_5bp`(공식)** · `pen_5bp_drop_50`(스트레스 하한)으로 병기한다.
TF는 **15m·1d 제외, 1h 조건부 채택(= 후속 최적화·검증을 돌리는 "작업 TF"), 4h 보류**
(OOS 심볼당 15.7거래로 WAN-84 유효 기준 20건 미달 → 판정 불가, 1h 검증 시 대조군으로 병기).
⚠️ **1h 조건부 채택은 "엣지가 있다"는 뜻이 아니다** — 공식 기준선 OOS 1h는 +5.71%(2/3),
최악 가정에선 +0.05%(1/3)로 사실상 0이다. **가장 덜 나쁜 후보**일 뿐이며 실거래 승인이
아니다(`ALPHABLOCK_LIVE_TRADING=false` 유지, [WAN-86 결정 2](docs/decisions/wan86.md)).
...
안 될 "스치듯 닿은 체결"에 몰려 있고, (2) **OOS 심볼 편중** — 공식 기준선 OOS 15m 평균
...
backtest.wan88_long_only_validation`). 공식 기준선(`pen_5bp`)에서 **유효 셀 16개 중
...
공식 기준선 1h OOS는 3/3 심볼 플러스(BTC +7.14%, ETH +17.43%, SOL +4.08%)지만, 그 수익이
...
`wan97_tf_fill_oos.csv`). 결론: **공식 기준선 `pen_5bp` · 15m/1d 제외 · 1h 작업 TF ·
...
⚠️ **오프셋 미채택 유지(`zone_limit_offset_bps=0.0`) — 단 WAN-99의 근거는 폐기됐다
(WAN-97)**: [`docs/decisions/wan97.md`](docs/decisions/wan97.md) §3
(원본: `backtest/reports/wan97_offset_recheck.csv`). WAN-99는 지정가를 가격이 오는
방향으로 마중 내보내 **관통 요구를 주문 가격으로 미리 만족시키는** 수단을 스윕하고
미채택을 권고했는데, 그 **핵심 논거가 교정 엔진에서 8배 틀렸다**:

- WAN-99: *"0 → 20bp까지 밀어도 체결률은 15m 기준 **+1.28%p**(13.56% → 14.84%)밖에 못
  산다 — 소수의 한계 체결을 사려고 전 거래에 세금을 매기는 셈"*.
- **교정 후 실측: +10.10%p(45.32% → 55.42%).** 첫 탭 면제가 살아나 체결이 3배로 늘자
  **오프셋이 살 수 있는 체결도 같이 늘었다.** "오프셋은 사실상 아무것도 못 산다"는
  전제가 무너졌으므로 **WAN-99의 논증을 재인용하지 말 것**.
- 실제로 전 구간에서 **2bp가 2TF × 2가정 4/4 전부에서 최댓값**이다(15m 공식 기준선
  +30.62% → **+34.52%**, 1h +28.02% → **+30.48%**). 교환비도 작은 오프셋에서만 유리하다
  (15m 0→2bp: 체결 +1.95%p / 승률 −0.15%p = **13.1×**, 0→20bp는 1.4×).
- **그럼에도 기본값을 바꾸지 않는다** — 이유는 "2bp가 나빠서"가 아니라 **오프셋 축의
  IS/OOS가 없어 과최적화를 배제할 수 없어서**다. 전 구간 최댓값을 그대로 채택하는 것은
  이 저장소가 반복해 경계해온 바로 그 함정이고, 이득(+2~4%p)이 시드·심볼 분산(−11%~+30%)
  보다 작다. **오프셋 × IS/OOS 재판정은 WAN-104** 소관이다.
- 큰 오프셋이 무너진다는 **구조적 세금 메커니즘 자체는 확인됐다**(15m 20bp **−52.12%**).
  오프셋의 문제는 "아무것도 못 산다"가 아니라 **"많이 사려 들면 파산한다"** 였다.
