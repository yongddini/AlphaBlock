...
누락」 참고. 15m 제외 결론의 재판정은 WAN-97 소관이다.
...
**최종 재판정은 WAN-97**이 오프셋 축·IS/OOS까지 포함해 전체 재산출로 수행한다.
WAN-99 CSV는 회귀 테스트가 고정한 1셀만 교정 엔진으로 갱신돼 **엔진이 섞여 있으니
셀 간 비교 금지**.
...
backtest.wan99_zone_limit_offset_report`). **아래 수치는 전부 WAN-100 교정 전 엔진
기준이다**(재판정은 WAN-97). WAN-96의 관통 요구를 **주문 가격으로 미리
