...
**→ WAN-96이 그 재검정을 수행했고, 결론은 15m 제외다**(아래). **WAN-99가 지정가
...
⚠️ **지정가 오프셋으로도 15m은 살아나지 않는다 — 오프셋 미채택(WAN-99)**:
`backtest/reports/wan99_zone_limit_offset_summary.md`(재현: `python -m
backtest.wan99_zone_limit_offset_report`). WAN-96의 관통 요구를 **주문 가격으로 미리
만족시키는** 반대편 수단(지정가를 가격이 오는 방향으로 마중 내보내기)을 스윕했다
(오프셋 0/2/5/10/20bp × 체결 가정 3종 × 3심볼 × 15m·1h × 3년, 390런).

- **15m: 아니오.** 최악 가정(`pen_5bp_drop_50`)에서 **5개 오프셋 전부 마이너스**이고,
  오프셋을 키울수록 단조롭게 악화된다(0bp −5.4% → 20bp −32.9%). WAN-96의 15m 제외
  권고는 오프셋으로 뒤집히지 않는다.
- **1h: 표면적으론 예, 그러나 채택 근거는 아니다.** 최악 가정 전 구간에서 5bp가
  +3.6%(3/3 심볼 플러스)로 보이지만 **두 관문을 다 떨어진다**: (1) 이웃이 무너지는
  **뾰족한 봉우리**(2bp +0.4%, 10bp −6.4% → 이웃 평균 −3.0%), (2) IS에서 고른 5bp의
  OOS 성적이 +0.1%로, **오프셋 0(+1.8%)보다 못하다**. 즉 오프셋이 보탠 게 없다.

**기본값을 바꾸지 않는다**(`zone_limit_offset_bps=0.0`). 0이면 입력 가격을 그대로
반환하므로 WAN-95/96 결과가 비트 단위로 재현된다.

교환이 왜 지독하게 불리한가(이 이슈의 핵심): **오프셋은 버는 쪽이 얇은 꼬리에만 걸리고
내는 쪽은 전 거래에 걸린다.** 0 → 20bp까지 밀어도 체결률은 15m 기준 **+1.28%p**(13.56%
→ 14.84%)밖에 못 산다 — 가격은 존에 닿을 때 대개 20bp쯤 우습게 지나쳐 버리므로, 새로
잡히는 체결은 "근단 20bp 이내까지 왔다 되돌아선" 드문 셋업뿐이다. 반면 오프셋은 이미
체결됐을 거래의 진입가까지 전부 나쁘게 만들어 1R을 키우고 고정 1.5R 익절을 멀리 밀어,
승률을 **6.59%p** 무너뜨린다(45.38% → 38.78%). 소수의 한계 체결을 사려고 전 거래에
세금을 매기는 셈이다.
