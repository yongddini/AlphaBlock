...
⚠️ **이 번복은 아래 TF 판단의 입력을 바꿨다** — WAN-97의 채택 TF는 `pen_5bp` 수치로
내려졌고 **TF 재판정은 아직 안 됐다**(WAN-104는 오프셋 축만 다뤘다). 아래 TF 규칙은
"유효하되 근거 렌즈가 바뀌었음" 상태다.
TF는 **15m·1d 제외, 1h 조건부 채택(= 후속 최적화·검증을 돌리는 "작업 TF"), 4h 보류**
(OOS 심볼당 15.7거래로 WAN-84 유효 기준 20건 미달 → 판정 불가, 1h 검증 시 대조군으로 병기).
⚠️ **1h 조건부 채택은 "엣지가 있다"는 뜻이 아니다** — OOS 1h는 새 공식 렌즈(`baseline`)
에서 +8.18%(2/3), 민감도 `pen_5bp`에서 +5.71%(2/3), 스트레스에선 +0.05%(1/3)로 사실상
0이다(WAN-104 격자, 오프셋 0). **가장 덜 나쁜 후보**일 뿐이며 실거래 승인이 아니다
(`ALPHABLOCK_LIVE_TRADING=false` 유지, [WAN-86 결정 2](docs/decisions/wan86.md)).
⚠️ **15m 제외 근거가 WAN-96과 다르다**: WAN-96의 근거(최악 가정 −5.8%·0/3)는 WAN-100
교정 엔진에서 **소멸했다**(+3.57%·3/3). 새 근거는 (1) **관통 민감도 10.3배 레버리지** —
거래의 6.1%만 걷어내면 수익의 62.8%가 증발(1h 5.6×, 4h 6.4×)해 수익이 실거래에서 가장
안 될 "스치듯 닿은 체결"에 몰려 있고, (2) **OOS 심볼 편중** — `pen_5bp` OOS 15m 평균
(+11.38%)의 대부분을 ETH(+34.54%)가 만들고 BTC는 마이너스(−3.81%)다(최악 가정에선 BTC
5시드 전부 마이너스). 결론만 같고 근거는 다르니 **WAN-96의 근거를 재인용하지 말 것**.
...
(15m 제외 결정의 독립적 보강 근거). **이 판정은 "전략이 손실"이라는 뜻은 아니다** —
...
누락」 참고. **재판정은 WAN-97이 교정 엔진으로 완료했다**([`docs/decisions/wan97.md`](docs/decisions/wan97.md)):
15m 제외 결론은 **유지되지만 근거가 통째로 바뀌었다**(WAN-96의 −5.8%·0/3은 소멸,
관통 민감도·OOS 단일 심볼 의존이 새 근거 — 위 📌 문단).
...
**최종 재판정은 WAN-97이 IS/OOS 축까지 포함해 완료했다** —
[`docs/decisions/wan97.md`](docs/decisions/wan97.md)(원본: `wan97_tf_fill_grid.csv`,
`wan97_tf_fill_oos.csv`). 결론: **당시 공식 기준선 `pen_5bp`(→ WAN-104가 `baseline`으로
번복) · 15m/1d 제외 · 1h 작업 TF ·
4h 보류**(위 📌 문단). WAN-99 CSV는 회귀 테스트가 고정한 1셀만 교정 엔진으로 갱신돼
**엔진이 섞여 있으니 셀 간 비교 금지**.
...
**아래 수치는 전부 WAN-100 교정 전 엔진 기준이다**(위 참고 — 재판정은 WAN-97).
...
- ⚠️ **다중 포지션이 OOS에서 이기는 유일한 유니버스(15m 포함)는 WAN-97이 제외한 TF가
  끌고 간다** — 15m 재판정(WAN-107)이 15m을 되살리면 이 판정은 그 위에서 다시 해야 한다.
