...
- **재지 않는 것**: (b) 위치 널(가짜 존 — WAN-248, 다른 질문) · 큐 우선순위 실측
  (틱·호가 WAN-98, Canceled).
- 📌 **「선별 대 가격」 갈래는 관문으로 걸지 않는다** — 사용자 결정 2026-08-21
  (*"앞으로 그 선별 vs 가격은 없애 · 싸게 사는 것도 실력이다"*). 그 결정을 문서에 못 박는
  것은 **WAN-352**(별도 이슈, 아직 미실행)이고, 이 모듈은 그 이슈가 **「이 관문을 안 거는
  첫 대상」으로 명시**한 자리라 새 경고문을 달지 않는다. ⚠️ 축을 **쟀는데 문제없다**는
  뜻이 아니라 **안 쟀고 그걸 결격으로 세지 않는다**는 뜻이다.
