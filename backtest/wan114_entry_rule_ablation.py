...
def write_summary(rows: Sequence[AblationRow], path: Path) -> None:
    ...
    lines = [
        ...
        *verdict(summary, steps, segment=SEGMENT_IS),
        ...
        "",
        "1. **RSI 게이트는 값을 더하지 않는다 — 뺀다.** `L0r→L1`이 12셀(2TF × 2구간 × 3렌즈) "
        "중 **8셀에서 음수**고, **1h는 6셀 전부 음수**(−1.12 ~ −2.29%p)다. 유일하게 일관된 "
        "플러스인 15m IS(+2.78%p)는 **OOS에서 −2.76%p로 뒤집힌다** — CLAUDE.md가 반복해 적은 "
        "IS→OOS 뒤집힘이 부품 축에서 재현됐다. 게이트는 거래를 13~14% 쳐내는데 **그 쳐냄이 "
        "손해**다.",
        "2. **재탭 노출도 값을 더하지 않는다.** `L0→L0r`은 12셀 중 **9셀에서 음수**다 — 거래를 "
        "~30% 늘리면서 15m OOS에서 −3.85%p(**0/6심볼 상승**), MDD는 +3.96%p 오른다. 플러스인 "
        "곳은 1h OOS(+2.19%p, 3/6)뿐이고 그 IS는 −1.94%p라 방향이 서로 반대다.",
        "3. **볼린저만 값을 더한다.** `L1→L2`가 12셀 중 **9셀에서 양수**이고 15m은 세 렌즈 "
        "전부에서 크게 살아남는다(OOS +11.60 ~ +17.93%p). 뒤집히는 곳은 1h OOS(−0.74 ~ "
        "−1.92%p)뿐이다.",
        "",
        "**그래서 이슈의 질문에 답하면**: 15m의 규칙 층 전체 기여(+9.59%p)는 **볼린저 하나의 "
        "몫(+16.20%p)이고 RSI·재탭이 −6.61%p를 깎아먹는다**. 1h에서는 규칙 층 전체가 "
        "마이너스라 **존-단독(`L0` +2.48%)이 채택 기본값(`L2` +1.22%)을 이긴다** — IS에서도 "
        "마찬가지다(+17.95% vs +16.70%).",
        "",
        "⚠️ **「그럼 존 자체가 값이다」로 읽지 말 것** — 15m OOS의 존-단독은 **−5.68%(2/6심볼)**"
        '다. 이 표는 "존은 좋은데 규칙이 망친다"가 아니라 **"존-단독에도 규칙에도 신호가 '
        '없고, 15m의 플러스는 볼린저의 진입가 재산정에서 온다"** 로 읽는다. WAN-88/111의 '
        "「엣지 없음」과 모순되지 않는다 — 그 판정을 **부품 단위로 분해**한 것이다.",
        "",
        "⚠️ **볼린저의 기여가 「선별」이라는 증거는 이 표에 없다 — 「가격」일 가능성이 크다.** "
        "볼린저는 진입가를 재산정하는 규칙이지 셋업을 고르는 규칙이 아니다(밴드가 존보다 "
        "불리하면 기각하는 WAN-75 규칙 3만 선별에 해당한다). 롱 진입가가 낮아지면 1R이 줄어 "
        "고정 1.5R 익절이 가까워지므로 **승률이 오르고**(15m OOS +3.23%p) MDD가 준다(−10.99%p) "
        '— 실제로 그렇게 움직인다. 이는 WAN-96의 진단("지정가 진입의 수익은 셋업이 좋아서가 '
        '아니라 더 좋은 가격에 들어가서 나온다")과 **같은 결론**이다. 선별 대 가격을 가르려면 '
        "「볼린저 기각(규칙 3)은 쓰되 진입가는 존 근단 그대로」인 단이 필요하다 — 이 표에는 "
        "없다(후속 이슈).",
        "",
        "⚠️ **채택 기본값의 체결률 ~51%는 시장이 아니라 규칙 층이 만든 것이다.** 존-단독은 "
        "**체결률 100%** 다 — 탭이 곧 존 근단 터치라 첫 탭 무조건 진입은 정의상 항상 체결된다. "
        "거기서 RSI 게이트가 100%→62%로, 볼린저 재산정이 62%→51%로 깎는다. WAN-95/96이 "
        '"체결률 ~50%가 낙관적이다"라고 걱정한 그 수치는 **규칙 층이 스스로 만든 미체결**이다.',
        "",
        "## 규칙 층 기여의 체결 의존도 — 렌즈별 `L0` → `L2` 격차",
        "",
        *lens_sensitivity(summary, segment=SEGMENT_OOS),
        "",
        "**규칙 층의 기여는 체결 낙관의 산물이 아니다** — 15m에서 렌즈를 조일수록 격차가 오히려 "
        "커진다(`baseline` +9.59%p → `pen_5bp` +11.97%p). 관통 요구가 존-단독(−12.73%)을 채택 "
        "기본값(−0.76%)보다 더 세게 때리기 때문이다. ⚠️ 단 **격차가 커지는 것과 돈을 버는 것은 "
        '다르다**: `pen_5bp`에서는 **두 단 모두 마이너스**다. 즉 볼린저는 "덜 나쁘게" 만들 뿐 '
        "민감도 렌즈에서 플러스를 만들지 못한다.",
        "",
