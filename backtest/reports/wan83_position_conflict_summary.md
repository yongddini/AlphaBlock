...
채택 기본값(`ConfluenceParams()` = 롱 온리·지정가·`first_tap_free`·고정 1.5R)에서, `tap_index=0`(무조건 진입)인데 포지션 보유로 스킵된 첫 탭이 존 무효화 전에 재탭됐는지, 재탭이 RSI 게이트를 통과했는지 분해한다. **이 이슈는 계측만 하며 진입 로직은 바꾸지 않는다.**
