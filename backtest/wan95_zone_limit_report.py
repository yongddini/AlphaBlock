...
- **지정가 진입 = 메이커**: `maker_fee_rate`, 슬리피지 없음.
- **청산은 사유별로 갈린다**(WAN-370): **익절**(부분 익절 포함)도 실거래에서 지정가
  reduce-only로 내므로 **메이커 · 슬리피지 0**이고, **손절·만료·데이터 종료**만 시장가
  (테이커 + 슬리피지)다.

🚨 **요율 값을 여기에 다시 적지 않는다** — 단일 소스는 `BacktestConfig.exit_liquidity`
(사유 → 유동성 구분)와 `maker_fee_rate`/`fee_rate`/`slippage`다(WAN-370). 값을 서술에
베껴 두면 다음 전환에서 또 갈라진다 — 이 독스트링이 실제로 그렇게 낡았고(WAN-370이 익절을
메이커로 옮겼는데 문장은 「청산은 테이커」로 남았다) WAN-429가 그것을 고쳤다.

⚠️ **이 절의 옛 문장(「청산은 테이커」·「왕복 0.11%」)을 재인용하지 말 것** — 숫자(CSV·요약
표)는 WAN-384 재산출로 **새 회계에서 나왔고** 낡았던 것은 **서술뿐**이다. 이 저장소가
반복해 겪은 「라벨과 동작이 어긋남」(WAN-91/95/112/123/159/194)의 **서술 축 변종**이다.

WAN-91이 "15m 채택 제외"를 **비용** 근거로 권고했는데, 그 전제(테이커 종가 진입)가
사용자의 실매매와 달랐다 — 이 리포트가 메이커 비용으로 다시 돌린 재산출이다(옛 손익표
`wan87_long_only_summary.md`·WAN-91 펀딩 리포트는 종가 진입 = **사용자가 하지 않는
매매**의 성과였다).
...
def build_markdown(frame: pd.DataFrame, proxy_note: str = "") -> str:
    ...
    lines = [
        ...
        "",
        "> 🔁 **WAN-384(존폭 필터 폐지)로 `zone_limit` 행이 다시 전면 재산출됐다 — 바로 위 "
        "WAN-159를 번복한다.** 채택 기본 `max_zone_width_atr`가 `1.28`(좁은 존만) → "
        "**`None`(꺼짐, 전부 매매)** 이 되면서 진입 **후보 집합이 통째로 넓어졌다**(이 표 "
        "기준 거래 수가 크게 는다). 근거는 WAN-378 §1b 격자다 — `oos_warm` · 가드 0.30% · "
        "재진입 ON · **거래당 net R**에서 문턱을 조일수록 나빠지고(`끔` −0.1207 > `1.28` "
        "−0.1309 > `0.80` −0.1651) 그 방향이 **진입가 3팔 · 구간 4개 전부** 같다. "
        "⚠️ **「끄면 이긴다」로 인용 금지** — `끔`도 **−0.1207R로 여전히 마이너스**이고"
        "(WAN-378의 108팔 전부 음수) `끔`과 `2.60`의 차이는 규약 폭(±0.005R) 안이라 "
        "구분되지 않는다. 고른 근거는 성적이 아니라 **자유 파라미터를 하나 없애는 것**이다 "
        "· ⚠️ **손절폭 가드(0.3%)와 혼동 금지** — 그쪽은 필터를 꺼도 **+0.1614R**로 존폭 축"
        "(−0.0101R)보다 16배 큰 일을 하고 이 결정이 건드리지 않았다(WAN-381 소관) "
        "· ⚠️ **「엣지 없음」(WAN-84/88/111/114/124/151/201/248)은 그대로**다. 근거·경고는 "
        "[`docs/decisions/wan384.md`](../../docs/decisions/wan384.md).",
        "",
        "## 무엇이 바뀌었나",
        "",
        "채택 기본값(`ConfluenceParams()`)의 진입 방식을 **종가 시장가 → 존 근단 지정가"
        "(`zone_limit`)** 로 바꾸고, 지정가 진입에 **메이커 수수료**를 배선했다. "
        "사용자의 실매매가 지정가이므로, 이전 손익표(`wan87_long_only_summary.md`, "
        "WAN-91 펀딩 리포트 등)는 **사용자가 하지 않는 매매**의 성과였다.",
        "",
        "📌 **이 표가 무는 비용**(요율 값은 여기에 적지 않는다 — 단일 소스는 "
        "`BacktestConfig.exit_liquidity`와 `maker_fee_rate`/`fee_rate`/`slippage`다, "
        "WAN-370): **진입(지정가)은 메이커 · 슬리피지 없음** · **익절(부분 익절 포함)도 "
        "메이커 · 슬리피지 0**(실거래에서 지정가 reduce-only로 내므로 — WAN-370) · "
        "**손절·만료·데이터 종료만 테이커 + 슬리피지**. 옛 종가 진입(전 구간 테이커)보다 "
        "싸고, 대신 **미체결 위험**이 생긴다(아래 체결률).",
        "",
        "> 🔁 **WAN-429: 이 절의 서술이 WAN-370(2026-08-25) 이후 낡아 있었다** — "
        "「청산은 테이커(익절 포함) · 왕복 0.11%」라고 적혀 있었는데 익절은 그때 이미 "
        "메이커로 옮겨진 뒤였다. 🚨 **숫자는 맞다** — 표·CSV는 WAN-384가 새 회계로 "
        "재산출했고 낡았던 것은 **문장뿐**이다(「라벨과 동작이 어긋남」 "
        "WAN-91/95/112/123/159/194의 **서술 축 변종**). 옛 문장을 재인용하지 말 것.",
        "",
        "## 방법론",
        "",
