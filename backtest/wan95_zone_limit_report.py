...
def build_markdown(frame: pd.DataFrame, delta: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        "",
        "> 🔁 **WAN-123(재탭 RSI 게이트 제거)으로 이 표가 다시 전면 재산출됐다 — 이번엔 "
        "`close` 행도 함께 움직인다.** 채택 기본 `rsi_gate_mode`가 `first_tap_free` → "
        "**`unconditional`**(게이트 자체가 없음)이 되면서 재탭에 걸리던 RSI 조건(롱 "
        "`RSI<=30`)이 사라졌다. 오프셋(WAN-112)이 **같은 거래의 가격**을 옮긴 것과 달리 이건 "
        "**거래 집합 자체를 넓히므로**(이 표 기준 지정가 거래 +19~27%) 양쪽 진입 방식이 다 "
        "이동했다 — 그래도 이 표가 격리하는 변수(진입 방식)는 두 팔이 **함께** 게이트를 "
        "잃으므로 그대로다. ⚠️ **체결률이 약 51% → 약 81%로 뛴 것이 게이트의 몫**이다: "
        ...
        "이동한다. `close` 행은 A안이라 **정의상 무관**하다(봉 단위에서는 `intrabar_live`가 "
        "`tap`과 같은 값이다) — 그래도 재산출 시점이 달라 `--years 3` 창이 미끄러진 만큼은 "
        "움직일 수 있다. ⚠️ **「수익이 올라서 바꿨다」로 인용 금지**(WAN-119/120 실측: 15m "
