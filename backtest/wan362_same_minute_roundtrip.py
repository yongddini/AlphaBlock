...
* ❌ **「백테가 손실을 숨겼다」가 아니다** — 백테는 그 셋업에 **진입하지 않는다**(취소).
  숨긴 손실이 아니라 **아예 없는 거래**다. 라이브가 더 하는 그 거래들의 크기는 이 표가
  재지 않는다(무효화 봉 안의 탭 인구조사 = 후속 이슈).
...
def render_summary(
    ...
    lines.append(
        ...
        "봉 끝에서야 알 수 있는 사실로 봉 처음의 주문을 걷어낸다. ⚠️ **엔진을 고치자는 제안이 "
        "아니다**(후보 집합이 바뀌는 재-베이스라인 = 사용자 결정) — 같은 분 손절이 7건뿐인 "
        "이유가 「1분봉이 순서를 모른다」가 아니라 **여기**라는 기록이다."
    )
    lines.append("")
    lines.append(
        "📌 **라이브는 그 반대 방향으로 어긋난다** — `live/limit_engine.py`가 스스로 적어 둔 "
        "알려진 근사: "
        "백테스트는 무효화 봉 **시작**에 취소하지만 라이브는 그 봉이 닫혀 탐지가 확인한 뒤에야 "
        "취소한다(한 봉 늦음). 즉 **라이브는 존을 깨는 봉 안에서 진입할 수 있고 그 진입은 곧바로 "
        "손절로 끝난다.** 페이퍼의 같은 분 손절이 어디서 오는지에 대한 정합적인 설명이다."
    )
    lines.append("")
    lines.append(
        "⚠️ **「백테가 손실을 숨겼다」로 인용 금지** — 백테는 그 셋업에 **진입하지 않는다**. "
        "숨긴 손실이 아니라 **없는 거래**다. 라이브가 더 하는 그 거래들의 크기(무효화 봉 안의 탭 "
        "인구조사)는 이 표가 재지 않는다 — 후속 이슈."
    )
    lines.append("")
