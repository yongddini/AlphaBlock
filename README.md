...
- **진입 조건** = 존 확정 후 **첫 탭은 RSI 무관 무조건 진입**(워밍업 NaN 포함),
...
⚠️ **다음 갈림길은 결정 대기 중(WAN-86)**: 숏 유지 여부·실거래 보류·다음 방향
3개 항목을 [`docs/decisions/wan86.md`](docs/decisions/wan86.md)에서 문서화하고
있다. 결정 전까지 전략 규칙을 바꾸는 코드 변경(파라미터 탐색 포함)은 열지 않는다.
