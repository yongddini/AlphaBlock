...
📌 **시작 점검은 최근 창만 본다 (WAN-187, 기본 7일)** — 갭 탐지가 시리즈를 통째로
pandas로 읽던 것을 저장소 커서 스트리밍으로 바꿨지만(1m 한 시리즈 실측 9.9초·RSS
2.3GB → 1.2초·17MB), 6년 × 9종목이면 전 구간 스캔만으로도 45 시리즈 ~40초(콜드)라
그만큼 **웹소켓 접속이 늦는다**. 시작 창은 `ALPHABLOCK_REPAIR_ON_START_LOOKBACK_DAYS`
로 조정하고(0이면 전 구간), 실측 기본 7일은 같은 DB에서 **0.07초**다.
⚠️ 창을 좁힌 실행의 「갭 없음」은 **「그 창 안에서」**라는 뜻이다 — 그래서 요약
(`RepairSummary.lookback_ms`)과 `alphablock status`가 창을 함께 찍는다. 전 구간
점검은 `alphablock backfill --repair`(창 없음)와 `alphablock verify`가 맡는다.
...
ALPHABLOCK_REPAIR_ON_START_LOOKBACK_DAYS=7       # 시작 점검이 보는 창(0=전 구간)
```
