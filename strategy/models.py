...
class ConfluenceParams(BaseModel):
    ...
    **기본 `24`(현행 동작 보존)**. `None`(WAN-73): 유효기간을 두지 않고 존이 무효화
    ...
    ⚠️ **단위는 ATR 배수지 퍼센트가 아니다** — 1.24는 "존폭이 ATR의 1.24배 이하"라는 뜻이다.
    퍼센트로 착각해 `0.0124` 같은 값을 넣으면 거의 모든 셋업이 걸러진다(WAN-112의
    `2.0` vs `0.0002` 함정과 같은 부류).

    📌 **기본값이 아니라 옵트인이다**(WAN-158 = 사용자 결정 B). WAN-142/152/154가 세 관문
    (표본 축소·기하·체결 보수화+문턱 민감도)을 통과시켰지만 사용자는 **먼저 켜 보고
    판단한다**를 골랐다 — 채택(A)이 아니다. 권고 문턱은 15m **1.24** · 1h **1.32**이고
    출처·경고(TRX 충돌 등)는 [`docs/decisions/wan158.md`](../docs/decisions/wan158.md).
    ...
    `None`이면 ATR 계산 자체를 하지 않으므로 기본 실행은 예전과 **비트 단위로 같다**.
    """
