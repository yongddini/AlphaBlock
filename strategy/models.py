...
class ConfluenceParams(BaseModel):
    ...
    * `"bar_open"` — WAN-364까지의 옛 동작(소급 취소). 무효화 봉의 `open_time`부터 취소된
    ...
    # --- 진입 근거 게이트: 존폭 (WAN-158 옵트인 → WAN-159 채택 기본값) ---
    max_zone_width_atr: float | None = Field(default=1.28, gt=0)
    """존폭 필터 — 존폭 ÷ ATR가 이 값 **이하**인 셋업만 진입한다. **기본 `1.28`(켜짐)**.
    ...
    📌 **채택 기본값이다**(WAN-159 = 사용자 결정, WAN-158 결정 B→A 승격). 좁은 존만 매매한다.
    문턱 `1.28`은 측정 권고값(15m **1.24** · 1h **1.32**)의 중간이고 — TF별 값을 두면 앞구간에
    맞춰 고르는 자유 파라미터가 하나 늘어(WAN-108) 하나로 통일했다 — **데이터가 고른 값이
    아니라 자유 파라미터를 안 늘리려는 사용자 판단**이다. 근거·경고(승률 상승의 과반은 기하 ·
    TRX 15m은 손절폭 가드에 잘려 16거래로 붕괴 · 거래가 3분의 1로 준다 · 「엣지 없음」은
    그대로)는 [`docs/decisions/wan159.md`](../docs/decisions/wan159.md).
    ...
    옛 수치를 결론에 박아 둔 리포트는 이 필드를 `None`(꺼짐)으로 **명시 고정**해 당시 엔진의
    기록으로 보존한다(`harness.LEGACY_MAX_ZONE_WIDTH_ATR`) — 고정하지 않으면 기본값을 따라
    조용히 필터 켜진 엔진으로 다시 돌아 본문과 어긋난다(WAN-149 `combine_obs`와 같은 부류).
    """
